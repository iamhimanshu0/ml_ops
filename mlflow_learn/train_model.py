...
train_dataset = dataset['train'].shuffle().select(range(10000)).map(func_tokenizer, batched=True)
test_dataset = dataset['test'].shuffle().select(range(1000)).map(func_tokenizer, batched=True)
